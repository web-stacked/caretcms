...
- **Integration** (`packages/core/src/index.ts`): `caret()` factory. Resolves config
  (mode `embedded`|`cloud`, storage/upload providers, `delivery` static|server),
  generates virtual modules (`virtual:caretcms/providers`, `virtual:caretcms/schemas`)
  via Vite plugins, injects `order:'pre'` middleware (server delivery) or build-time
  HTML bake (static delivery), ~16 routes in dev/server mode, and the inline-editor
  bootstrap script.
